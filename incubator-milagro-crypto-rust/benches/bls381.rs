...
fn basic_signing(c: &mut Criterion) {
    ...
    c.bench(
        "basic",
        Benchmark::new("Create a Signature G2 on BLS381", move |b| {
            b.iter(|| {
    ...
    c.bench(
        "basic",
        Benchmark::new("Verify a Signature G2 on BLS381", move |b| {
            b.iter(|| {
...
fn aggregate_verfication_n(n: i32, c: &mut Criterion) {
    let mut rng = create_rng();
    let mut pks_g1 = Vec::with_capacity(n as usize);
    let mut pks_g2 = Vec::with_capacity(n as usize);
    let mut sks_g1 = Vec::with_capacity(n as usize);
    let mut sks_g2 = Vec::with_capacity(n as usize);
    let mut msgs = Vec::with_capacity(n as usize);
    let mut sigs_g1 = Vec::with_capacity(n as usize);
    let mut sigs_g2 = Vec::with_capacity(n as usize);
    ...
    // c.bench(
    //     "aggregation",
    //     Benchmark::new(format!("Verifying aggregate of {} signatures G1", n).as_str(), move |b| {
    //         b.iter(|| {
    ...
    c.bench(
        "aggregation",
        Benchmark::new(format!("Verifying aggregate of {} signatures G2 on BLS381", n).as_str(), move |b| {
            b.iter(|| {
...
fn aggregate_verfication(c: &mut Criterion) {
    aggregate_verfication_n(10, c);
    aggregate_verfication_n(100, c);
    aggregate_verfication_n(1000, c);
}
